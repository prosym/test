...
//SJISは良くないですよ    fwrite($file, e2s($data));
    ...
    $subject = "46th programming symposium registration";
    $message = e2j(
	"第46回プログラミング・シンポジウムの参加申し込みを，\n" .
	"以下の登録内容で受け付けました．\n\n" .
	...
	"たばこ        " . $smoke . "\n" .
	"その他        " . $p_sonota . "\n\n" .
	"請求書，郵便振替用紙などを12月中頃に郵送します．\n" .
	"登録内容変更，キャンセル連絡，その他お問い合わせは，\n" .
	"prosym46@ipsj.or.jp までお願いします．\n" .
	"キャンセルの期限は，12月17日(金)です．" 
	);
    $headers = "From: prosym46@ipsj.or.jp";
    $parameters = "-f webmaster@kitty.watalab.cs.uec.ac.jp";
    $ok = (
	"受け付けました．<br>" . 
	"登録内容をメールで送信しました．<br>" .
        "メールが届かない場合は" .
	"prosym46@ipsj.or.jpにお問い合わせください．"
	);
    $ng = (
	"何らかのエラーが発生しました．<br>" .
	"prosym46@ipsj.or.jpにお問い合わせください．"
    ...
    if (mail($p_email, $subject, $message, $headers, $parameters)) {
        mail("prosym46@ipsj.or.jp", $subject, $message, $headers, $parameters);
        print($ok);
    } else {
        print($ng);
    }
