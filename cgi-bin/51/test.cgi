...
#        読み込む                #
#--------------------------------#
my $c = new Config::Simple('test.ini')
...
# パラメータ指定があれば開いてみる
    ...
    print $q->h2(Encode::decode_utf8($cfg->param('User.name')), ' 様', $q->br, $q->br, '発表申込みはこれで完了いたしました。', $q->br,
                 '採否の結果は、幹事会での審議を経て、11月1日にお知らせいたします。', $q->br,
                 'また、原稿の提出〆切は11月27日の予定です。', $q->br, $q->br, 
                 'お申込みありがとうございます。');
    print $q->h3('ご不明な点がございましたら',
		 $q->a({href=>'mailto:' . $mail}, $mail),
		 'までお問い合わせください。');
...
sub send_mail_acceptance {
    ...
    $mesg .= '採否の結果は、幹事会での審議を経て、11月1日までにお知らせいたします。' . "\n";
    $mesg .= 'また、原稿の提出〆切は11月27日の予定です。お申込みありがとうございます。' . "\n\n";
