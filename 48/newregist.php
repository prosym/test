...
//	if(!checkdate($Psdaymonth,$Psdayday,$Psdayyear)){    $error[] = '送金/振込み日';}
	...
	if($Prcpt==null) { $error[] = '領収書'; $Keyrcpt=1;}
	if($Psmoke==null) { $error[] = '喫煙調査'; $Keyrcpt=1;}
...
/*- 初期フォーム表示 -------------------------------------*/
	...
	tr_Making('年齢',$Keyage);		form_Pulldown(Page,10,80,$_SESSION['Page'],10);
		print "歳台&nbsp;&nbsp;（部屋割りの参考にさせて頂きますが，必須ではありません）";
...
//		form_Radio(Penter,'発表学生',$Penter);			print "<hr size=1 />";
		form_Radio(Penter,'発表学生',$_SESSION[Penter]);
		form_Radio(Penter,'６５歳以上の発表者',$_SESSION[Penter]);			print "<hr size=1 />";
	...
	tr_Making(add_flg($Needroom,'希望する部屋 <br /> (学生は4名1室)'),$Keyroom);
